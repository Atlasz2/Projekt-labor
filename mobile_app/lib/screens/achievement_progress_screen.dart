...
class AchievementProgressScreen extends StatefulWidget {
  ...
  @override
  State<AchievementProgressScreen> createState() =>
      _AchievementProgressScreenState();
}
...
class _AchievementProgressScreenState extends State<AchievementProgressScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final events =
          ((progressData['completedEvents'] as List<dynamic>?) ?? []).length;
  ...
  _ProgressData _progressFor(Map<String, dynamic> ach) {
    ...
    final target = (_safeInt(ach['conditionValue']) <= 0)
        ? 1
        : _safeInt(ach['conditionValue']);
              ...
              child: ListView(
                ...
                children: [
                  ...
                  ..._sortedAchievementViews().map((view) {
                    ...
                    final unlocked = _unlockedIds.contains(
                      (a['id'] ?? '').toString(),
                    );
                            ...
                            Wrap(
                              ...
                              children: [
                                Container(
                                  ...
                                  child: Text(
                                    'Feltetel: ${progress.conditionLabel}',
  ...
  List<_AchievementView> _sortedAchievementViews() {
    final list = _achievements
        ...
        .toList();
    list.sort((a, b) {
      final aUnlocked = _unlockedIds.contains(
        (a.achievement['id'] ?? '').toString(),
      );
      final bUnlocked = _unlockedIds.contains(
        (b.achievement['id'] ?? '').toString(),
      );
