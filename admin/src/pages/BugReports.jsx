...
const SEVERITY_LABEL = { low: "Alacsony", medium: "Kozepes", high: "Magas" };
const SEVERITY_COLOR = { low: "success", medium: "warning", high: "error" };
const STATUS_LABEL = { open: "Nyitott", closed: "Lezart" };
...
function BugReports() {
  ...
  const fetchReports = async () => {
    try {
      ...
      setError("Betoltesi hiba: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showSnack("Torlesi hiba: " + err.message);
    }
  };
  ...
  const handleToggleStatus = async (report) => {
    ...
    try {
      ...
      showSnack("Frissitesi hiba: " + err.message);
    }
  };
  ...
  const handleSaveResponse = async (report) => {
    ...
    try {
      ...
      showSnack("Mentesi hiba: " + err.message);
    } finally {
      setSavingId(null);
    }
  };
    ...
    <Box sx={{ p: 3, maxWidth: 960, mx: "auto" }}>
      <Box sx={{ mb: 4, pb: 3, borderBottom: "1px solid", borderColor: "divider" }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2.5 }}>
          Felhasznalok altal bekuldott hibajelentesek kezelese es megvalaszolasa.
        </Typography>
        <Stack direction="row" gap={1.5} flexWrap="wrap">
          {[
            { label: "Osszes", value: reports.length, bg: "#eff6ff", color: "#2563eb", border: "rgba(37,99,235,0.2)" },
            ...
            { label: "Lezart", value: reports.filter(r => normalizeStatus(r.status) === "closed").length, bg: "#f0fdf4", color: "#059669", border: "rgba(5,150,105,0.2)" },
      ...
      <Stack direction="row" gap={2} alignItems="center" mb={3} flexWrap="wrap">
        <FormControl size="small" sx={{ minWidth: 150 }}>
          <InputLabel>Szuro</InputLabel>
          <Select value={filter} label="Szuro" onChange={(e) => { setFilter(e.target.value); localStorage.setItem("admin_br_filter", e.target.value); }}>
            <MenuItem value="all">Osszes ({reports.length})</MenuItem>
            <MenuItem value="open">Nyitott ({reports.filter(r => normalizeStatus(r.status) === "open").length})</MenuItem>
            <MenuItem value="closed">Lezart ({reports.filter(r => normalizeStatus(r.status) === "closed").length})</MenuItem>
          </Select>
        </FormControl>
        <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchReports} size="small">Frissites</Button>
      </Stack>

      {filtered.length === 0 && (
        <Alert severity="info">Nincs {filter !== "all" ? STATUS_LABEL[filter]?.toLowerCase() : ""} hibajelentes.</Alert>
      )}
          ...
          <Card key={r.id} variant="outlined" sx={{ borderLeft: "4px solid", borderLeftColor: normalizeStatus(r.status) === "closed" ? "grey.400" : SEVERITY_COLOR[r.severity] === "error" ? "error.main" : SEVERITY_COLOR[r.severity] === "warning" ? "warning.main" : "success.main" }}>
            <CardContent>
              <Stack direction="row" alignItems="flex-start" justifyContent="space-between" gap={2} flexWrap="wrap">
                <Box flex={1}>
                  <Typography variant="subtitle1" fontWeight={700}>{r.title || "(Cim nelkul)"}</Typography>
              ...
              {r.admin_response && responseInputs[r.id] === undefined && (
                <Box mt={1.5} p={1.5} sx={{ background: "#f0f7ff", borderRadius: 2, borderLeft: "3px solid #2563eb" }}>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>Admin valasz:</Typography>
              ...
              <Box mt={2}>
                <TextField
                  ...
                  placeholder="Valasz a felhasznalonak..."
                />
              </Box>
            </CardContent>
            ...
            <CardActions sx={{ px: 2 }}>
              <Button size="small" startIcon={normalizeStatus(r.status) === "closed" ? <RadioButtonUncheckedIcon /> : <CheckCircleIcon />} onClick={() => handleToggleStatus(r)}>
                {normalizeStatus(r.status) === "closed" ? "Ujranyitas" : "Lezaras"}
              </Button>
              ...
              <Box flex={1} />
              <Button size="small" color="error" startIcon={<DeleteIcon />} onClick={() => setDeleteId(r.id)}>Torles</Button>
            </CardActions>
          </Card>
        ))}
      </Stack>
      ...
      <ConfirmDialog
        open={deleteId !== null}
        title="Hibajelentes torlese"
        message="Biztosan torli ezt a hibajelentest? Ez a muvelet nem vonhato vissza."
        confirmText="Torles"
